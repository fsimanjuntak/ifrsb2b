...
object FdvMoboPmUnearned {
  def main(args: Array[String]): Unit = {
    ...
    val fdvMoboPmUnearned  = Common.getConfigDirectory(sc, configDir, "FDV.OUTPUT.FDV_MOBO_PM_UNEARNED")
    
    sqlContext.read.parquet(fdvMoboPmJoin+"/PRC_DT="+prcDt).registerTempTable("FDV_MOBO_PM_JOIN")
    
    
    
    val fdvMoboPmUnearnedDf = sqlContext.sql(""" SELECT MOBO_TRX_ID, 
                                                       MOBO_TRX_DTTM,
                                                       MOBO_VOUCHER_SN,
                                                       MOBO_VOUCHER_EXP_DT,
                                                       MOBO_INJ_DEALER_ID,
                                                       ...
                                                       """+prcDt+""" PRC_DT
                                                FROM FDV_MOBO_PM_JOIN WHERE PM_VOUCHER_STATUS = 'E'
    """)
    Common.cleanDirectory(sc, fdvMoboPmUnearned+"/PRC_DT="+prcDt)
     fdvMoboPmUnearnedDf.repartition(10)
    .write.partitionBy("PRC_DT")
    .mode("append")
    .parquet(fdvMoboPmUnearned)
