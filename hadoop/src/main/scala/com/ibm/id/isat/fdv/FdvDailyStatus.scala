...
object FdvDailyStatus {
  def main(args: Array[String]): Unit = {
    ...
    val fdvDailyStatus  = Common.getConfigDirectory(sc, configDir, "FDV.OUTPUT.FDV_DAILY_STATUS")
    
    sqlContext.read.parquet(fdvMoboPmJoin+"/PRC_DT="+prcDt).registerTempTable("FDV_MOBO_PM_JOIN")
    
     val fdvDailyStatusDf = sqlContext.sql("""
        SELECT  MOBO_TRX_DTTM,
         ...
         GROUP BY 
                ...
                PM_VOUCHER_STATUS
    """)
    Common.cleanDirectory(sc, fdvDailyStatus+"/PRC_DT="+prcDt)
      fdvDailyStatusDf.repartition(10)
    .write.partitionBy("PRC_DT")
    .mode("append")
    .parquet(fdvDailyStatus)
