...
object FdvAsOffOutlet {
  def main(args: Array[String]): Unit = {
    ...
    val fdvAsOffOutlet  = Common.getConfigDirectory(sc, configDir, "FDV.OUTPUT.FDV_ASOFF_OUTLET")
    
    sqlContext.read.parquet(fdvMoboPmJoin+"/PRC_DT="+prcDt).registerTempTable("FDV_MOBO_PM_JOIN")
      
     val fdvAsOffOutletDf = sqlContext.sql(""" SELECT """+prcDt+""" MOBO_TRX_DTTM,
                                  PM_VOUCHER_STATUS,
                                  MOBO_INJ_DEALER_ID,
                           ...
                           GROUP BY 
                                  ...
                                  MOBO_INJ_OUTLET_NAME
    """)
    Common.cleanDirectory(sc, fdvAsOffOutlet+"/PRC_DT="+prcDt)
    fdvAsOffOutletDf.repartition(10)
    .write.partitionBy("PRC_DT")
    .mode("append")
    .parquet(fdvAsOffOutlet)
