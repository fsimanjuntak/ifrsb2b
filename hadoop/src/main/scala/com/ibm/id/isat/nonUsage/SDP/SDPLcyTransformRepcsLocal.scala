...
/**
 * This object is used to produce revenue from SDP Lifecycle Reprocess
 * Insert into SDP Lifecycle detail
 * Insert into SDP Lifecycle summary
 * @author Meliani Efelina
 */
    
object SDPLcyTransformRepcsLocal {
    ...
    def main(args: Array[String]) {
        ...
        println("Start Processing Reprocess SDP Lifecycle");  
        ...
        val SOURCE1 = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.SOURCE_REJ_REF1_DIR");
        val existDir1= Common.isDirectoryExists(sc, SOURCE1)

        val SOURCE2 = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.SOURCE_REJ_REF2_DIR");
        val existDir2= Common.isDirectoryExists(sc, SOURCE2)
        
        //val sdpLcySrcRDD = sc.textFile(source);
        //val sdpLcyRejRef1RDD = sc.textFile(SOURCE1.concat("*/*"));
        //val sdpLcyRejRef2RDD = sc.textFile(SOURCE2.concat("*/*"));
        ...
        val configDir = "/user/apps/nonusage/sdp/conf/SDP.conf"
        val OUTPUT_DETAIL_REV_DIR = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.OUTPUT_SDP_DETAIL_DIR")
        val OUTPUT_REJ_REF1_DIR = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.OUTPUT_SDP_REJ_REF1_DIR").concat(CHILD_DIR)
        val OUTPUT_REJ_REF2_DIR = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.OUTPUT_SDP_REJ_REF2_DIR").concat(CHILD_DIR)
        val OUTPUT_REJ_REF1_PERM = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.OUTPUT_SDP_REJ_REF1_PERM_DIR").concat(CHILD_DIR)
        val OUTPUT_REJ_REF2_PERM = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM_LOCAL.OUTPUT_SDP_REJ_REF2_PERM_DIR").concat(CHILD_DIR)
        
        
        
        /*
         * variable sdpLcyInputRDD1
         * This variable contains reject reference 1 data that will be used in the next process of SDP Lifecycle
         * Directory will be checked first before do the splitting. If not exist, default RDD will be created.
         */
        if (sdpLcyRejRef1RDD.count > 0) {
        val sdpLcyInputRDD1 = sdpLcyRejRef1RDD.filter ( x => x.count (_ == '|') == 22).map(line => line.split("\\|")).map { p => Row(  
               p(0), //oriNodeTp
               p(1), //timeStamp
               p(2), //subsNum
               p(3), //svcClsId
               ...
               val sdpLcyInputDF1 = sqlContext.createDataFrame(sdpLcyInputRDD1, model.SDPLcy.SDPLcySchema)
               sdpLcyInputDF1.registerTempTable("RejRef1SDPLcy")
               sqlContext.cacheTable("RejRef1SDPLcy")
               sdpLcyInputDF1.filter(" datediff(now(),concat(substring(timeStamp,1,4),'-',substring(timeStamp,5,2),'-',substring(timeStamp,7,2))) <= 90 ").registerTempTable("STGSDPAdjustment")
               sdpLcyInputDF1.filter(" datediff(now(),concat(substring(timeStamp,1,4),'-',substring(timeStamp,5,2),'-',substring(timeStamp,7,2))) > 90 ")
        ...
        else
        {
               ...
               val sdpLcyInputDF1 = sqlContext.createDataFrame(tempRDDRow, model.SDPLcy.SDPLcySchema)
               sdpLcyInputDF1.registerTempTable("RejRef1SDPLcy")
        ...
        /*
         * variable sdpAdjInputRDD2
         * This variable contains reject reference 2 data that will be used in the next process of SDP Lifecycle
         * Directory will be checked first before do the splitting. If not exist, default RDD will be created.
         */
        if (sdpLcyRejRef2RDD.count > 0) {
        val sdpLcyInputRDD2 = sdpLcyRejRef2RDD.filter ( x => x.count (_ == '|') == 24).map(line => line.split("\\|")).map { p => Row(  
               ...
               val sdpLcyInputDF2 = sqlContext.createDataFrame(sdpLcyInputRDD2, model.SDPLcy.SDPLcySubRejSchema)
               sdpLcyInputDF2.registerTempTable("RejRef2SDPLcy")
               sqlContext.cacheTable("RejRef2SDPLcy")
               sdpLcyInputDF2.filter(" datediff(now(),concat(substring(transaction_date,1,4),'-',substring(transaction_date,5,2),'-',substring(transaction_date,7,2))) <= 90 ").registerTempTable("STGSDPAdjustment")
               sdpLcyInputDF2.filter(" datediff(now(),concat(substring(transaction_date,1,4),'-',substring(transaction_date,5,2),'-',substring(transaction_date,7,2))) > 90 ")
               .write.format("com.databricks.spark.csv").mode("overwrite").option("delimiter", "|").save(OUTPUT_REJ_REF1_PERM);
        ...
        else
        {
               ...
               val sdpLcyInputDF2 = sqlContext.createDataFrame(tempRDDRow2, model.SDPLcy.SDPLcySubRejSchema)
               sdpLcyInputDF2.registerTempTable("RejRef2SDPLcy")
               //sdpLcyInputDF2.show()
        }      
        ...
        val ha1SDPLcyDF = sqlContext.sql(
            ...
            from
               (SELECT * FROM RejRef1SDPLcy 
               where timeStamp <> '' and subsNum <> '' and origTimeStmp <> '' and svcClsId <> ''
               and ((acFlagsBef = 10001110 and acFlagsAf = acFlagsAf) or (acFlagsBef = 10001111 and acFlagsAf = 00000000))
                )  sdp
               ...
               (substring(timeStamp,1,8) between svcClassEffDt and svcClassEndDt )
             """)
         ...
         * prvdr_id 24, country 25, promo_package_code 27, promo_package_nm 28, brand_nm 29,  hlr_branch_cd 30
        ...
        // define columns for sub sdp lifecycle class and create data frame.
       ...
       println("============= Union ================");  
    		...
    		// combine row from sdp lifecycle and sdp lifecycle sub
            ...
            UNION ALL
            select transaction_date TRANSACTION_DATE, subsNum, prefix, city, provider_id, country,
               svcClsId, promo_package_code, promo_package_name, brand_name, 
               total_amount ,
               revenue_code,
               real_filename,
           ...
           sqlContext.cacheTable("SDPLcyUnion")
        ...
        finalTransformDF.registerTempTable("SDPLcyUnionTransform");
         Common.cleanDirectoryWithPattern(sc, OUTPUT_DETAIL_REV_DIR, "/*/*/JOB_ID=" + JOB_ID);
        finalTransformDF.where("SERVICE_TYPE is not null and REVENUEFLAG is not null").write.partitionBy("SRC_TP","TRANSACTION_DATE","JOB_ID").mode("append").save(OUTPUT_DETAIL_REV_DIR)
        	  ...
        	  select TRANSACTION_DATE, MSISDN, PREFIX, CITY, PROVIDER_ID, COUNTRY, SERVICECLASSID, 
                   ...
                   case 
        	         ...
        	         from SDPLcyUnionTransform
        		       where SERVICE_TYPE is null or REVENUEFLAG is null
    				""");
