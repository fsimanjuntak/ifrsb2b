...
object FdvAsOffStatus {
  def main(args: Array[String]): Unit = {
    ...
    val fdvAsOffStatus  = Common.getConfigDirectory(sc, configDir, "FDV.OUTPUT.FDV_ASOFF_STATUS")
    
    sqlContext.read.parquet(fdvMoboPmJoin+"/PRC_DT="+prcDt).registerTempTable("FDV_MOBO_PM_JOIN")
  
    
    val fdvAsOffStatusDf = sqlContext.sql("""
        SELECT  """+prcDt+""" MOBO_TRX_DTTM,
         ...
         GROUP BY 
                ...
                PM_VOUCHER_STATUS
    """)
    Common.cleanDirectory(sc, fdvAsOffStatus+"/PRC_DT="+prcDt)
     fdvAsOffStatusDf.repartition(10)
    .write.partitionBy("PRC_DT")
    .mode("append")
    .parquet(fdvAsOffStatus)
