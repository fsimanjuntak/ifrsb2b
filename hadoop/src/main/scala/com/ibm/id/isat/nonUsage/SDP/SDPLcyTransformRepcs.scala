...
object SDPLcyTransformRepcs {
    ...
    def main(args: Array[String]) {
        ...
        val SOURCE1 = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM.SOURCE_REJ_REF1_DIR");
        val existDir1= Common.isDirectoryExists(sc, SOURCE1)

        val SOURCE2 = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM.SOURCE_REJ_REF2_DIR");
        ...
        val OUTPUT_REJ_REF1_PERM = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM.OUTPUT_SDP_REJ_REF1_PERM_DIR").concat(CHILD_DIR)
        val OUTPUT_REJ_REF2_PERM = Common.getConfigDirectory(sc, CONF_DIR, "SDP.SDP_LCY_TRANSFORM.OUTPUT_SDP_REJ_REF2_PERM_DIR").concat(CHILD_DIR)
        ...
        val sdpLcyInputRDD2 = sdpLcyRejRef2RDD.filter ( x => x.count (_ == '|') == 24).map(line => line.split("\\|")).map { p => Row(  
               ...
               p(9), //brand_name
               Common.toDouble(p(10)), //total_amount
               p(11), //revenue_code
               p(12), //real_filename
               p(13), //hlr_branch_nm
               p(14), //hlr_region_nm
       ...
       println("============= Union ================");  
         ...
         */
    		val finalTransformDF = sqlContext.sql(
                   ...
                   """+JOB_ID+""" JOB_ID,
                   rcrd_id RECORD_ID,
                   cast ("""+PRC_DT+""" as string) as PRC_DT,
                   srcTp SRC_TP,
                   fileDt FILE_DT
