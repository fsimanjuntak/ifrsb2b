...
object FdvDailyOutlet {
  def main(args: Array[String]): Unit = {
    ...
    val fdvDailyOutlet  = Common.getConfigDirectory(sc, configDir, "FDV.OUTPUT.FDV_DAILY_OUTLET")
    
    sqlContext.read.parquet(fdvMoboPmJoin+"/PRC_DT="+prcDt).registerTempTable("FDV_MOBO_PM_JOIN")
    
    val fdvDailyOutletDf = sqlContext.sql(""" SELECT MOBO_TRX_DTTM,
                                  PM_VOUCHER_STATUS,
                                  MOBO_INJ_DEALER_ID,
                           ...
                           GROUP BY 
                                  ...
                                  MOBO_INJ_OUTLET_NAME
    """)
    Common.cleanDirectory(sc, fdvDailyOutlet+"/PRC_DT="+prcDt)
     fdvDailyOutletDf.repartition(10)
    .write.partitionBy("PRC_DT")
    .mode("append")
    .parquet(fdvDailyOutlet)
