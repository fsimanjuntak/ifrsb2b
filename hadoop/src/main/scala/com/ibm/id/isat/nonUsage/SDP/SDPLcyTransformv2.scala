...
object SDPLcyTransformv2 {
    ...
    def main(args: Array[String]) {
        ...
        val sdpLcyInputRDD = sdpLcySrcRDD.filter ( x => x.count (_ == '|') == 70).map(line => line.split("\\|")).map { p => Row(  
               ...
               if ((p(17) == "10001111" && p(18) == "00000000") || 
                   (p(17) == "10001110" && p(18) == "10001111") || 
                   (p(17) == "00000000" && p(18) == "10000000")
                   )
                  if (p(50).count (_ == '[') >= 1 ) 
                     ...
                     "MA".concat("#").concat(p(56)).concat("#").concat(p(57)).concat("#").concat(p(16))
               else
                 "",//daDtl
               JOB_ID, //jobId
