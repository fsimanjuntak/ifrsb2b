...
//    prepaidReSummaryDf.registerTempTable("prepaid_re_summary")
    ...
    where allocated = 'N' and line_amt <> 0
    group by
        prc_dt,
        start_dt,
        prepaid_re_summary.customer_segment,
        ...
        allocated
    """)
    sapPrepaidNonIfrsDf.registerTempTable("sap_prepaid_non_ifrs")
    ...
    select
        ...
        case when sum(line_amount) < 0 then (- sum(line_amount)) else sum(line_amount) end amount_currency,
        first(business_area) business_area,
        ...
        case customer_segment when 'B2C PREPAID' then first(b2c_desc)
            when 'B2C SALMO' then first(b2c_desc)
            when 'B2B PREPAID' then first(b2b_desc) end long_text,
        sap_prepaid_non_ifrs.src_tp,
        first(sap_prepaid_non_ifrs.description) description,
        first(unearned_gl_account) unearned_gl_account
    from sap_prepaid_non_ifrs
    left join ref_sid_distribution
    on lower(sap_prepaid_non_ifrs.rev_code) = lower(ref_sid_distribution.sid)
    and (case when sap_prepaid_non_ifrs.SRC_TP <> 'SALMO' then 'NONSALMO' else 'SALMO' end) = ref_sid_distribution.src_tp
    group by
        customer_segment,
        profit_center,
        b2c_gl,
    ...
    val sapLineDf = sqlContext.sql("""
        ...
        where assignment_assignment is not null and amount_currency > 0
    """)
    sapLineDf.persist()
    ...
    select 
        ...
        first(posting_date) posting_date,
        'IDR' currency_key,
        --profit_center reference_reference,
        reference_reference reference_reference,
        --'Prepaid Non-IFRS Impacted' document_text,
        document_text document_text,
    ...
    sapDf.coalesce(1)
      ...
      .option("header", "true")
      .option("delimiter", "|")
      .save(pathOutputSap+"/PRC_DT=" + prcDt + "/hdp_prepaid_YB")
