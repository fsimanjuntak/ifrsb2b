...
object SDPAdjTransformv2  {
    ...
    def main(args: Array[String]) {
        ...
        val sdpAdjInputRDD = sdpAdjSrcRDD.filter ( x => x.count (_ == '|') == 96).map(line => line.split("\\|")).map { p => Row(  
               ...
               "", //msisdnPad
               p(72), //subAdj
               if (p(0) == "1" && p(72) == "" && p(18) == "0000000001" && p(19) == "0000000001") 
                 "MA".concat("#").concat(p(7)).concat("#").concat(p(8)).concat("#").concat(p(9))
               else if (p(0) == "1" && p(72) != "" && p(18) == "0000000001" && p(19) == "0000000001") 
         ...
         * Null New Service Class, and Invalid Adjustment Record Type
         */
        val ha0SDPAdjDF = sqlContext.sql(
            """
            SELECT adjRcrdTp, 
               sdp.subsNum,
               sdp.adjTimeStamp,
            ...
            from
             ...
             """);
          ha0SDPAdjDF.registerTempTable("HA0SDPAdjustment");
          ha0SDPAdjDF.persist()
