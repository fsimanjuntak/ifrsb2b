...
//    val pathRefDistribution = "C:/Users/KharisSimon/Box Sync/Indosat/IFRS/Sample/spark/ref_distribution"
    ...
    select
        ...
        gl_account_b2b_desc b2b_gl_desc,
        gl_account_b2c_desc b2c_gl_desc,
    ...
    val sapPostpaidIfrsDf = sqlContext.sql("""
    select
        prc_dt,
        job_id,
    ...
    where allocated = 'Y' and revenue_mny <> 0
    group by
        prc_dt,
        job_id,
        start_dt,
        ...
        account_type_name
    """)
    sapPostpaidIfrsDf.registerTempTable("sap_postpaid_ifrs")
    ...
    select
        ...
        case when sum(revenue_mny * nvl(share,1)) < 0 then '40' else '50' end posting_key,
        clearing_gl_account gl_account,
        case when sum(revenue_mny * nvl(share,1)) < 0 then (- sum(revenue_mny * nvl(share,1))) else sum(revenue_mny * nvl(share,1)) end amount_currency,
        business_area,
        profit_center,
        'ID00' business_place,
        ...
        case customer_segment when 'B2C MOBILE' then b2c_gl else b2b_gl end long_text
    from sap_postpaid_ifrs
    left join ref_sid_distribution on lower(sap_postpaid_ifrs.revenue_code_name) = lower(ref_sid_distribution.sid)
    ...
    select reference_key_1, document_date, profit_center, reference_reference, business_area
    from sap_non_vat_base
    group by reference_key_1, document_date, profit_center, reference_reference, business_area
    having sum(amount_currency) <> 0
    """)
    .repartition(1)
    distinctReferenceDf.registerTempTable("distinct_reference")
    val refReferenceToDocIdDf = broadcast(sqlContext.sql("""
    select
        reference_key_1, document_date, profit_center, reference_reference, business_area,
    ...
    select 
        ...
        case when long_text is null then sap_non_vat_base.reference_reference else long_text end long_text
    from sap_non_vat_base
    join reference_to_doc_id
        on sap_non_vat_base.reference_key_1 = reference_to_doc_id.reference_key_1
        ...
        and sap_non_vat_base.reference_reference = reference_to_doc_id.reference_reference
        and  sap_non_vat_base.business_area = reference_to_doc_id.business_area
    where amount_currency > 0
    """)
    sapNonVatLineDf.persist()
    ...
    sapNonVatCsvDf.coalesce(1)
      ...
      .option("header", "true")
      .option("delimiter", "|")
      .save(pathOutputSap+"/PRC_DT=" + prcDt + "/hdp_postpaid_YC")
    
    rootLogger.info("====job ifrs SAP postpaid IRFS finished====")
  }
